...
static INSTANCE: OnceLock<Blockchain> = OnceLock::new();
...
impl Blockchain {
    ...
    fn init(evm: EVM) -> Result<(), ChainError> {
        INSTANCE
            .set(Blockchain::new(evm))
            .map_err(|_| ChainError::Custom("Blockchain instance already initialized".to_string()))
    }

    pub fn get_instance() -> Result<&'static Blockchain, ChainError> {
        INSTANCE
            .get()
            .ok_or_else(|| ChainError::Custom("Blockchain instance not initialized".to_string()))
    }
